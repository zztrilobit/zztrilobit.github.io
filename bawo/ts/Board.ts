...
class Board {
/*
Class Board is the Bawo board. It contains methods to make moves,
evaluate board instances, and determine best moves.
*/
    /* initial number of tokens in nyumba (yokhoma mode) */
    static INIT_TOKENS : number = 10;
...
*/
    ...
    side : number; /* 0=lower, default human; 1=upper, default computer */
...
/* the value of the current board as returned by the eValuate() method is the
    ...
    value :number [];
    private static LIM : number =Integer.MAX_VALUE;
	
	new_byte(len: number):number[]{
	  var i:number,  res:number[] =[];
	  for(i=0;i<len;i++) res.push(0);
	  return res;
	}
	new_int(len: number):number[]{
	  var i:number, res:number[]=[];
	  for(i=0;i<len;i++) res.push(0);
	  return res;
	}
	new_boolean(len: number):boolean[]{
	  res=[];
	  for(i=0;i<len;i++) res.push(false);
	  return res;
	}
    constructor() {
        this.state=this.new_byte(33);
        this.move=this.new_byte(33);
        this.value=this.new_int(2);
        this.status=this.new_boolean(2);
        this.side=0;
        this.gameOver=false;
        this.contCapture=false;
    }
    ...
    copy(old:Board):void {
        ...
        i:number;
        for (i=0;i<33;i++) this.state[i]=old.state[i];
        for (i=0;i<2;i++) this.status[i]=old.status[i];
        this.side=old.side;
        this.gameOver=old.gameOver;
        this.contCapture=old.contCapture;
        this.contTakata=old.contTakata;
    }

    updateMove (d1r: number):void {
        /**********************************************************************
        ...
        **           Fixed bug #3085082.
        **********************************************************************/
        var i,index,dir,n,dest,mtaji=-1,mtajiCount=0;//int
        var mkatiTokens=0;//byte
        var s=side;//byte
        var takata:boolean;

        if (this.gameOver) for (i=0;i<32;i++) this.move[i]=-1;
        else do {
            s=(byte) ((s+1)%2);
		...
		if (state[index]>15 && state[32]==0 || s==side && contTakata)
                    ...
                    for (dir=-1;dir<2;dir+=2) {
                        ...
                        dest=(index+n*dir)%16;
                        if (dest<0) dest+=16;
                        if (n>1 && dest<8 && state[dest+s*16]>0 && state[7-dest+((s+1)%2)*16]>0) 
						{ /* capture==true */
...
*/
    ...
    updateState(index:number, dir:number):void {
        /**********************************************************************
        ** Created:  UNKNOWN by Zangaphee Chimombo
        ** Based On:
        ** Purpose:  
        ...
        ** Changes:
        **********************************************************************/
        var  i:number, currPtr:number; /* 0..15 */
        byte n=0;
...
// maybe gameOver shd b > LIM?
	...
	/* if root has only one move return it */
        if (numMoves==1 && Bawo.depth==depth) return sid;
        if (numMoves<2) Bawo.msg("error!");
