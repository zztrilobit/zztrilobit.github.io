...
// i think initial board state should be in Board.java
    ...
    void reSet(boolean p1Starts) {
        ...
        updateMove(0);
        if (p1Starts) Bawo.m=null;
        else {
            if (Bawo.p2Level.getSelectedItem()!="human") {
                Bawo.m=new Move();
            }
	}
    }
    ...
    synchronized void animate(int index) {
        ...
        Bawo.d.updateBowl(index,java.awt.Color.lightGray);
        do {
            try {
                Thread.sleep(Bawo.delay);
            }
            catch (InterruptedException exc) { }
        } while (Bawo.pause_status);
        // Bawo.bowls[index].changeBackGround(java.awt.Color.white);
        Bawo.d.updateBowl(index,java.awt.Color.white);
    }

    void messages() {
        if (gameOver) {
            if (value[0]>0) Bawo.msg("\nPlayer 1 wins...");
            else Bawo.msg("\nPlayer 2 wins...");
...
//    }
    ...
    void takeBack() {
        ...
        if (Bawo.m!=null && Bawo.m.isAlive()) return;
        if (moveNo>takeBackNo+1 && !gameOver) {
            ...
            for (int i=0;i<33;i++) {
                ...
                prevState2[i] = -1;
                prevState1[i] = -1;
                Bawo.d.updateBowl(i,java.awt.Color.white);
            }
            updateMove(0);
            Bawo.msg("\nTake Back");
        }
