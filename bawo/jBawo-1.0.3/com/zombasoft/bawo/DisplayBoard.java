...
//	    g.drawRect(h+SPACING,v+SPACING-DIA,DIA-2*SPACING,-2*SPACING+2*DIA);
    ...
    public void updateBowl(int index, Color bgColour) {
        ...
        paintTokens(g, index, Bawo.p.state[index]);
    }
...
//	for (int i=0; i<34; i++)
	for (int i=0; i<33; i++) {
            ...
            paintTokens(g, i, Bawo.p.state[i]);
        }
    }
...
//        else if (index<32 || index==33) return 0;
...
/*            switch (j) {
    ...
    public void mousePressed(MouseEvent event) {
        ...
        if (Bawo.m==null && Bawo.p.move[index]>-1
	&& (Bawo.p.side==0 && index<16 || Bawo.p.side==1 && index>15
        || Bawo.playMode.getSelectedIndex()==0)) {
            Bawo.m = new Move(index);
	}
    }

    public void mouseReleased(MouseEvent event) {
        if (Bawo.m==null) return;
        Bawo.m.mouseReleased();
    }
