...
class TsoroSide
    ...
    init:()->
        ...
        for i in [1..@cell_count]
            @data_inner.push(@seed_count)
            @data_outer.push(@seed_count)
        return this
    
    #доступ к ячейкам
    get_cell:(c,r)->@dt[r][@ind(c)]
    set_cell:(c,r,v)->@dt[r][@ind(c)]=v
    inc_cell:(c,r)->++@dt[r][@ind(c)]
    extract_cell:(c,r)->
        res=@dt[r][@ind(c)]
        @dt[r][@ind(c)]=0
        res
    ...
    fill:(b) ->  
        b.man=@man        
        for i in [1..@cell_count]
            b.data_inner[i-1]=@data_inner[i-1]
            b.data_outer[i-1]=@data_outer[i-1]
        return this
...
class TsoroBoard
    constructor: (cell_count,seed_count,continue_move)->
        ...
        @seed_count=seed_count
        @field=[]
        @field[1]=new TsoroSide(cell_count,seed_count,1)
        @field[2]=new TsoroSide(cell_count,seed_count,2)
        @continue_move=continue_move
        @gover=NO
    
    template:()-> return new TsoroBoard(@cell_count,0,@continue_move)
