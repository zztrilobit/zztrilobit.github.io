...
class ReversiBoard
    ...
    fill: (dest) ->
        dest.counts[1]=@counts[1]
        dest.counts[2]=@counts[2]
    ...
    do_move: (m,side) ->
        ...
        @counts[side]+=m.flips.length
        @counts[3-side]-=m.flips.length

    score: () ->
        scoreX=0
        scoreO=0
        return (sx:@counts[1],so:@counts[2])
    
    score_side: (s) ->
        return @counts[s]

    delta_side: (s) ->
        return @counts[s]-@counts[3-s]
...
class Heuristic
    ...
    pre_filter:(board,side,pm)->
        ...
        if not done and l1.length>0 
            z=l1 #можешь поставить на борт -поставь
            done=1==1
        if not done and cntr.length>0 
            z=cntr #можешь поставить в центр -поставь
            done=1==1
        if not done and l2.length>0 
            z=l2 #на вторую линию
            done=1==1
        if not done and pc.length>0 
            z=pc #на вторую линию
            done=1==1
        if z.length==0
            z=pm
...
class Reversi2
    ...
    onCellClick: (i,j) ->
        ...
        if flips.length>0
            @rb.do_move((y:i,x:j,flips:flips),1)
